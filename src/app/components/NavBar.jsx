    ...
    return (
        <>
            ...
            <button
                ...
                onClick={toggleMenu}>
                <span
                    className={`block w-5 h-0.5 bg-dark-color  ${
                        isOpen ? "rotate-45 translate-y-1/2" : "-translate-y-1"
                    }`}></span>
                <span
                    className={`w-5 h-0.5 bg-dark-color  ${
                        isOpen ? "hidden" : "block"
                    }`}></span>
                <span
                    className={`block w-5 h-0.5 bg-dark-color  ${
            ...
            <nav
                ...
                fixed top-64 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-pallete-color5/90 rounded-lg`}>
                {getNavLinks(true)}
            </nav>

            {/* Navigation Bar for large screen (Laptops and Desktops). */}
            <nav className="w-1/3 flex flex-row justify-between items-center px-4 md:hidden sm:hidden">
                {getNavLinks()}
            </nav>
        </>
    );
